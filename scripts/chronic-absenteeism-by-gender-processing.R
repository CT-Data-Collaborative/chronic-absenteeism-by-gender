...
backfill_years <- expand.grid(
  ...
  `Gender` = unique(chronic_absent_dist$`Gender`),
  `Year` = years 
)
...
complete_chronic_absent <- merge(chronic_absent_fips, backfill_years, by = c("FixedDistrict", "Gender", "Year"), all=T)

complete_chronic_absent <- complete_chronic_absent[order(complete_chronic_absent$FixedDistrict, complete_chronic_absent$Year, complete_chronic_absent$`Gender`),]
...
test <- complete_chronic_absent_long[,c("District", "Year", "Gender")]
test2<-test[duplicated(test), ]

#Write CSV
write.table(
  complete_chronic_absent_long,
  file.path(path_to_top_level, "data", "chronic_absenteeism_by_gender_2012-2016.csv"),
  sep = ",",
  row.names = F
)